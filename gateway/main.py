...
def init_upload(req: InitUploadReq):
    ...
    with db.begin() as cx:
        cx.execute(text("INSERT INTO files(file_id, owner_id, filename, size_bytes) VALUES (:f, :o, :n, :s)"),
...
def list_download(file_id: str):
    # return presigned GET per chunk, ordered by offset
    with db.begin() as cx:
        rows = cx.execute(text("SELECT offset, sha256, s3_key FROM chunks WHERE file_id=:f ORDER BY offset ASC"),
