...
import { X, UserPlus, Mail, Shield, Users as UsersIcon, Eye, Settings as SettingsIcon, Trash2, File } from 'lucide-react';
import type { Team, TeamMember, UserRole } from '../../types/teams';
...
export const TeamManagementModal: React.FC<TeamManagementModalProps> = ({ team, isAdmin, onClose, onUpdate }) => {
  const [activeTab, setActiveTab] = useState<'members' | 'files' | 'settings'>('members');
  ...
  const currentUserMember = team.members.find(m => m.id === 'current-user-id');
  const canUpload = currentUserMember ? ROLE_PERMISSIONS[currentUserMember.role].canUploadFiles : false;
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-white/90">
                ...
                <span className="text-sm font-medium">{exactFileCount} files shared</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <nav className="flex space-x-1">
            ...
            >
              ...
              <span className={`ml-1 px-2 py-0.5 text-xs font-semibold rounded-full ${
                activeTab === 'members' ? 'bg-primary-100 text-primary-700' : 'bg-gray-200 text-gray-600'
              }`}>
                {team.members.length}
              </span>
            </button>
            ...
            >
              ...
              Files
              <span className={`ml-1 px-2 py-0.5 text-xs font-semibold rounded-full ${
                activeTab === 'files' ? 'bg-primary-100 text-primary-700' : 'bg-gray-200 text-gray-600'
              }`}>
                {team.sharedFiles}
              </span>
            </button>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {isAdmin ? (
                                <select
                                  ...
                                  className={`px-2 py-0.5 rounded text-xs font-medium border-0 focus:outline-none focus:ring-1 focus:ring-primary-500 ${getRoleBadge(member.role)}`}
                                >
                                  <option value="viewer">Viewer</option>
                                  <option value="member">Member</option>
                                  <option value="admin">Admin</option>
                                ...
                                <span className={`px-2 py-0.5 rounded text-xs font-medium inline-flex items-center gap-1 ${getRoleBadge(member.role)}`}>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(ROLE_PERMISSIONS).map(([role, perms]) => {
                    ...
                    const permissions = Object.entries(perms)
                      ...
                      .map(([key]) => key.replace('can', '').replace(/([A-Z])/g, ' $1').trim().toLowerCase());

                    return (
                      <div key={role} className="bg-white rounded-lg p-4 border border-gray-200">
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium mb-3 ${getRoleBadge(role as UserRole)}`}>
                          <RoleIcon className="h-3 w-3" />
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => onUpdate({
                        ...team,
                        settings: { ...team.settings, allowMemberInvites: !team.settings.allowMemberInvites }
                      })}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                        team.settings.allowMemberInvites ? 'bg-primary-600' : 'bg-gray-200'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          team.settings.allowMemberInvites ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => onUpdate({
                        ...team,
                        settings: { ...team.settings, allowFileSharing: !team.settings.allowFileSharing }
                      })}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                        team.settings.allowFileSharing ? 'bg-primary-600' : 'bg-gray-200'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          team.settings.allowFileSharing ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => onUpdate({
                        ...team,
                        settings: { ...team.settings, requireApproval: !team.settings.requireApproval }
                      })}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                        team.settings.requireApproval ? 'bg-primary-600' : 'bg-gray-200'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          team.settings.requireApproval ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
