...
export interface FileShare {
  ...
  sharedWith: string;
  sharedAt: string;
  encryptedKey: string; // Re-encryption key (RK)
  permissions: 'read' | 'write';
  fileName?: string; // File name from JOIN
  fileSize?: number; // File size from JOIN
  mimeType?: string; // MIME type from JOIN
  ownerPublicKey?: { publicKey1: string; publicKey2: string } | string; // Owner's public key for re-encryption
